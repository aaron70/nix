...
in {
  ...
  flake.features.${name} = self.lib.mkFeature name ({
    pkgs,
    cfg,
    ...
  }: {
    ...
    config = {
      ...
      packages = with pkgs; let
        goPakcages =
          if cfg.configurations.go.enable
          then [
            go
            goperf
          ]
        ...
        rustPackages =
          if cfg.configurations.rust.enable
          then [
            cargo
            rustc
          ]
        ...
        pythonPackages =
          if cfg.configurations.python.enable
          then [
        ...
        nodejsPackages =
          if cfg.configurations.node.enable
          then [
            nodejs
          ]
          else [];
